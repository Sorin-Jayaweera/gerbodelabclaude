%% run_stripe_top_sweep.m
% Run sinusoidal driving from TOP (Y-direction) on STRIPE crystal
% Complements run_stripe_domain_sweep.m which drives from the side (X)
%
% Author: Gerbode Lab
% Date: 2026
...
% Same frequencies as side-driven stripe for comparison
...
fprintf(fid, '==========================================\n\n');
fprintf(fid, 'PURPOSE:\n');
fprintf(fid, '  TOP-driven (Y-direction) sinusoidal boundary driving.\n');
fprintf(fid, '  STRIPE domain structure for comparison with side-driven.\n\n');
fprintf(fid, 'DRIVING PARAMETERS:\n');
fprintf(fid, '  Drive direction: Y (vertical, from top edge)\n');
fprintf(fid, '  Drive amplitude: %.1f pixels\n', drive_amplitude);
fprintf(fid, '  Drive width: %d pixels from TOP edge\n', drive_width);
fprintf(fid, '  Fixed width: %d pixels from BOTTOM edge (absorbing BC)\n', fixed_width);
...
%% ==================== RUN SWEEP ====================
fprintf('============================================\n');
fprintf('   TOP-DRIVEN STRIPE FREQUENCY SWEEP\n');
fprintf('============================================\n');
...
for i = 1:length(frequencies)
    f = frequencies(i);
    sim_name = sprintf('stripetop_f%.4f_a%.1f', f, drive_amplitude);
...
for idx = 1:length(pending_indices)
    ...
    fprintf('\n----------------------------------------------\n');
    fprintf('Running %d/%d: f = %.4f (stripe top-driven)\n', ...
...
fprintf('\n============================================\n');
fprintf('TOP-DRIVEN STRIPE SWEEP COMPLETE!\n');
