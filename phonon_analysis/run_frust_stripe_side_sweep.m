%% run_frust_stripe_side_sweep.m
% Sinusoidal driving from SIDE (X-direction) on FRUSTRATED STRIPE crystal
% Uses initialize_grains_frust(sim) to create frustrated domain structure
% Wide geometry: 800 wide x 400 tall
%
% Author: Gerbode Lab
% Date: 2026
...
fixed_width = 25;      % Fixed region width (pixels from right edge)
num_frames = 20000;
...
% Crystal parameters - WIDE geometry
sim_width  = 800;
sim_height = 400;
looseness = 1.06;
zmax = 0.45;
...
%% ==================== RUN FREQUENCY SWEEP ====================
fprintf('============================================\n');
fprintf('FRUSTRATED STRIPE - SIDE-DRIVEN SWEEP\n');
fprintf('Geometry: %d x %d (wide)\n', sim_width, sim_height);
fprintf('Drive: X-direction from LEFT edge\n');
...
for i = 1:length(frequencies)
    f = frequencies(i);
    sim_name = sprintf('frust_f%.4f_a%.1f', f, drive_amplitude);
...
for idx = 1:length(pending_indices)
    ...
    sim_name = sprintf('frust_f%.4f_a%.1f', drive_frequency, drive_amplitude);
    sim_full_path = fullfile(simulations_folder, sim_name);

    fprintf('--------------------------------------------\n');
    fprintf('Sim %d/%d: f=%.4f (X-direction, frust stripe)\n', idx, length(pending_indices), drive_frequency);
    fprintf('Output: %s\n', sim_full_path);
    fprintf('--------------------------------------------\n');
...
fprintf('\n============================================\n');
fprintf('FRUST STRIPE SIDE-DRIVEN SWEEP COMPLETE!\n');
