%% run_random_domain_sweep.m
% Run sinusoidal driving simulations with RANDOM domain structure
...
%% ==================== BATCH FOLDER SETUP ====================
batch_name = 'randomsinesims';  % RANDOM domain simulations
...
fprintf(fid, '==========================================\n\n');
fprintf(fid, 'PURPOSE:\n');
fprintf(fid, '  Sinusoidal boundary driving with RANDOM domain structure.\n');
fprintf(fid, '  Study wave scattering at random grain boundaries.\n\n');
...
%% ==================== RUN SWEEP ====================
fprintf('==============================================\n');
fprintf('   RANDOM DOMAIN FREQUENCY SWEEP\n');
fprintf('==============================================\n');
...
for i = 1:length(frequencies)
    f = frequencies(i);
    sim_name = sprintf('sinusoidal_f%.4f_a%.1f_random', f, drive_amplitude);
...
for idx = 1:length(pending_indices)
    ...
    % Initialize crystal with RANDOM domains
    ...
    for frame = 1:num_frames
        ...
        if mod(frame, image_saving_frequency) == 0
            ...
            imshow(img);
            title(sprintf('RANDOM | f=%.4f | Frame %d | Cycle %.2f | Drive: %.2f px', ...
